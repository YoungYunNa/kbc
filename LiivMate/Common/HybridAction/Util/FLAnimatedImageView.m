...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        self.displayLink.preferredFramesPerSecond = kDisplayRefreshRate / MAX([self frameDelayGreatestCommonDivisor] * kDisplayRefreshRate, 1);
...
//            self.accumulator += displayLink.duration * displayLink.frameInterval;
            self.accumulator += displayLink.duration * (kDisplayRefreshRate / displayLink.preferredFramesPerSecond);
            ...
            if ([self.debug_delegate respondsToSelector:@selector(debug_animatedImageView:waitingForFrame:duration:)]) {
                [self.debug_delegate debug_animatedImageView:self waitingForFrame:self.currentFrameIndex duration:(NSTimeInterval)displayLink.duration * (kDisplayRefreshRate / displayLink.preferredFramesPerSecond)];
            }
#endif
        }
    } else {
