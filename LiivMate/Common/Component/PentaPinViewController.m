...
@interface PentaPinViewController ()<UITextFieldDelegate, VPFidoLibDelegate>
{
...
@implementation PentaPinViewController
@synthesize subTitle = _subTitle;
@synthesize infoMsg = _infoMsg;
@synthesize pentaTf = _pentaTf;
...
+(void)initialize
{
	...
	if(_sKey == nil)
	{
		unsigned char sessionKey[DEFAULT_KEY_SIZE];
		[PentaKeypad generateKey:sessionKey keySize:DEFAULT_KEY_SIZE];
		_sKey = [[NSData alloc] initWithBytes:sessionKey length:DEFAULT_KEY_SIZE];
	}
}
...
#ifdef USE_BLOCKCHAIN
    if ([@"Y" isEqualToString:[AppInfo sharedInfo].isBlockChainEnable]) {
        [PentaPinViewController initializeBlockChain];
        if(_opSignService == nil) return;
        @try {
            //공개키추출 성공
            if([_opSignService getPublicKey])
...
-(void)registCertAsync
{
    ...
    if ([@"Y" isEqualToString:[AppInfo sharedInfo].isBlockChainEnable]) {
        ...
        @try {
            ...
            [PentaPinViewController sendFailReport:@"1" bcaUsage:@"KATBCA0100" failReason:@"2"];
        }
    }
#endif
}
...
- (void)getPlainText:(PlainText)completion {
    //보안키패드 복호화
    const unsigned char *sKey = (void *)[_sKey bytes];
    
    NSString *encString = [self.pentaTf inputText];
    
    const char* _encString = [encString UTF8String];
    int _encLength = (int)[encString lengthOfBytesUsingEncoding:NSUTF8StringEncoding];
    if(_encLength % 2)
    {
        [NSException raise:NSGenericException format:@"[KBPentaSecureKeypad decrypt]: encString(%@) is not even", encString];
    }
    int byteSize = _encLength/2;
    char encBytes[byteSize];
    for(int i=0; i<byteSize; i++)
    {
        char tmpEnc[3] = {_encString[2*i],_encString[2*i+1],0};
        encBytes[i] = (char)strtol(tmpEnc,NULL,16);
    }
    ...
    unsigned char decBytes[byteSize+1];
    for(int i=0; i<byteSize; i++)
    {
        unsigned int decLen = 1;//byteSize-i;
        memset(iv,0,keySize);
        int iw_ret = IW_DecryptExBin(decBytes+i, &decLen, sKey, keySize, alg, encBytes+i, 1, 4, iv, IW_NPAD);
        if(iw_ret != IW_SUCCESS)
        {
            [NSException raise:NSGenericException format:@"[KBPentaSecureKeypad decrypt]: decryption error, %@", encString];
        }
        if(decLen != 1)
        {
            [NSException raise:NSGenericException format:@"[KBPentaSecureKeypad decrypt]: decryption error(len??), %@", encString];
        }
        decBytes[i+1] = 0;
    }
    completion((char *)decBytes);
...
{
	if(key == nil) return @"";
	NSString *encVal = [NSString stringWithFormat:@"timestamp=%.0f&%@_doubly_encrypted_hexa=%@",++PwdWrapper.timeStamp, key, dataStr];
	NSLog(@"VALUE : %@", encVal);
	
	char encodedEncData[8192] = {0,};
	int ret =
//	IW_HybridEncryptEx(encodedEncData,
...
//					   ALG_SEED);
	...
	IW_HybridStdEncrypt(encodedEncData,
						sizeof(encodedEncData),
						(void *)[_sKey bytes],
						(int)[_sKey length],
						[encVal UTF8String],
	...
	return [NSString stringWithFormat:@"%s", encodedEncData];
}

+(NSString*)encryptedDictionary:(NSDictionary*)dic
{
    NSInteger baseBufSize = 8192;
	if([dic isKindOfClass:[NSDictionary class]] == NO) return @"";
	NSString *encVal = [NSString urlQueryString:dic];
	encVal = [encVal stringByAppendingFormat:@"&timestamp=%.0f",++PwdWrapper.timeStamp];
	NSLog(@"VALUE : %@", encVal);
    
    NSInteger encryptedlength = ([encVal length]*2 + 200) > baseBufSize ? ([encVal length]*2 + 200) : baseBufSize;
    char *encodedEncData = (char *)malloc(encryptedlength);
    memset(encodedEncData, 0, encryptedlength);
    
//    char encodedEncData[8192] = {0,};
    NSLog(@"start");
	int ret = IW_HybridStdEncrypt(encodedEncData,
								  (int)encryptedlength,
								  (void *)[_sKey bytes],
								  (int)[_sKey length],
								  [encVal UTF8String],
    ...
    NSString * retStr = [NSString stringWithFormat:@"%s", encodedEncData];
    free(encodedEncData);
...
{
	[self showPenta:settingCallBack target:nil callBack:callBack];
}

+(void)showPenta:(PentaPinSetting)settingCallBack target:(NSString*)targetRequestID callBack:(PentaPinFinish)callBack
{
	PentaPinViewController *vc = [[UIStoryboard storyboardWithName:@"Component" bundle:nil] instantiateViewControllerWithIdentifier:@"PentaPinViewController"];
...
-(void)setupWithCallBack:(PentaPinFinish)callBack
{
	_callBack = callBack;
...
- (void)viewDidLoad
{
	...
	if([[UIDevice currentDevice] respondsToSelector:NSSelectorFromString(@"orientation")])
...
//            if(useBlockChain)
//            {
//                [Request requestID:P01203 body:nil waitUntilDone:NO showLoading:NO cancelOwn:nil finished:^(NSDictionary *result, NSString *rsltCode, NSString *message) {
//                    if(IS_SUCCESS(rsltCode))
//                    {
...
//            }
	...
	UITapGestureRecognizer *tapGes = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showPentaKeyPad:)];
    ...
    _fidoTitleLabel.text = isFaceID() ? @"Face ID를 사용하여" : @"지문을 사용하여";
	...
	keypadH.constant = CGRectGetHeight(self.pentaTf.inputView.frame);
...
-(void)showPentaKeyPad:(UITapGestureRecognizer*)ges
{
	if(_fidoContentView.hidden)
	{
		if(self.pentaTf.isFirstResponder == NO)
			[self.pentaTf becomeFirstResponder];
	}
}
...
-(void)becomeActiveNotification:(NSNotification*)noti
{
	...
	if(_successTouchID == NO && _showTouchID == NO)
...
-(PentaTextField*)pentaTf
{
	if(_pentaTf == nil)
	{
		_pentaTf = [[PentaTextField alloc] initWithFrame:CGRectMake(-100, 0, 60, 30)];
		_pentaTf.keyboardType = UIKeyboardTypeNumberPad;
		_pentaTf.textAlignment = NSTextAlignmentCenter;
		_pentaTf.maxLength = _maxLen;
		_pentaTf.alpha = 0;
		[_pentaTf addTarget:self action:@selector(controlEventEditingChanged:) forControlEvents:(UIControlEventEditingChanged)];
		[_pentaTf setEncryptionKey:(unsigned char *)_sKey.bytes keyLength:(int)_sKey.length];
		[self.view addSubview:_pentaTf];
	}
	return _pentaTf;
}

-(void)setPentaTf:(PentaTextField *)pentaTf
{
	if(pentaTf == nil)
	{
		[_pentaTf removeFromSuperview];
	}
	_pentaTf = pentaTf;
}
...
-(void)controlEventEditingChanged:(PentaTextField*)textField
{
	[self reloadLabel];
	if(textField.text.length >= _maxLen)
	{
		_showTouchID = NO;
		[self finishPentaPin:nil];
	}
}

-(IBAction)finishPentaPin:(id)sender
{
	if(_pentaTf.text.length < _maxLen)
	{
...
-(IBAction)onClickedPentaCloseButton:(id)sender
{
	[self cancelPentaPin];
}
...
-(void)reloadLabel
{
	...
	for(UIImageView *view in _imageViewArray)
	{
		view.highlighted = view.tag < _pentaTf.text.length;
	}
}

-(NSString *)encText
{
	return [_pentaTf inputText];
}
...
-(NSString*)doublyEncrypted:(NSString*)keyNm
{
	...
	return [PentaPinViewController doublyEncrypted:[_pentaTf inputText] key:keyNm];
}
		...
		else
		{
			...
			{//PIN+blockChain
                ...
                if ([@"Y" isEqualToString:[AppInfo sharedInfo].isBlockChainEnable]) {
                    ...
                    if(failReport)
                    {
                        [PentaPinViewController sendFailReport:@"2" bcaUsage:bcaUsage failReason:@"5"];
                        verifyMode = @"0";
                        bcaUsage = @"";
                        bcaReferrerKey = @"";
                ...
                if (!(error.code == kLAErrorUserCancel || error.code == kLAErrorSystemCancel))    // -2: Canceled by user, -4: UI canceled by system
                {
                    ...
                    if (resultMessage.errorCode == 0 && error == nil) {
                        [BlockAlertView showBlockAlertWithTitle:nil message: isFaceID() ? FaceIdLockoutMsg : TouchIdLockoutMsg dismissedCallback:^(BlockAlertView *alertView, NSInteger buttonIndex) {
                            ...
                            [self->_pentaTf performSelector:@selector(TranskeyBecomeFirstResponder) withObject:nil afterDelay:0.2];
                            self->_pinContentView.userInteractionEnabled = YES;
        ...
        switch (error.code) {
            case kLAErrorBiometryLockout:
            {
                ...
                [BlockAlertView showBlockAlertWithTitle:nil message: isFaceID() ? FaceIdLockoutMsg : TouchIdLockoutMsg dismissedCallback:^(BlockAlertView *alertView, NSInteger buttonIndex) {
                    ...
                    self->_showTouchID = NO;
                    self->_fidoContentView.hidden = YES;
                    [self->_pentaTf performSelector:@selector(becomeFirstResponder) withObject:nil afterDelay:0.2];
                    self->_pinContentView.userInteractionEnabled = YES;
