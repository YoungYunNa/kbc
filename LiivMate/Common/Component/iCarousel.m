...
- (iCarousel*)parentCarousel
{
	UIResponder *responder = self;
	while (![responder isKindOfClass:[iCarousel class]]) {
		responder = [responder nextResponder];
		if (nil == responder) {
			break;
		}
	}
	return (iCarousel *)responder;
}

-(void)accessibilityElementDidBecomeFocused
{
	iCarousel *carousel = [self parentCarousel];
	[carousel scrollToItemAtIndex:[carousel indexOfItemView:self] animated:NO];
...
//{
//	CGRect rect = [super accessibilityFrame];
//	if(self.currentItemView)
//	{
//		rect = CGRectInset(rect, (CGRectGetWidth(self.currentItemView.frame)-CGRectGetWidth(rect))/2,
//						   (CGRectGetHeight(self.currentItemView.frame)-CGRectGetHeight(rect))/2);
//	}
//	return rect;
//}
//
...
NSComparisonResult compareViewDepth(UIView *view1, UIView *view2, iCarousel *self)
{
	...
	if (difference == 0.0f)
	{
		...
		if (self.vertical)
		{
			float y1 = t1.m12 + t1.m22 + t1.m32 + t1.m42;
			float y2 = t2.m12 + t2.m22 + t2.m32 + t2.m42;
			float y3 = t3.m12 + t3.m22 + t3.m32 + t3.m42;
...
- (CGFloat)minScrollDistanceFromOffset:(CGFloat)fromOffset toOffset:(CGFloat)toOffset
{
	...
	if (_wrapEnabled)
	{
		float wrappedDistance = fminf(toOffset, fromOffset) + _numberOfItems - fmaxf(toOffset, fromOffset);
...
- (void)startDecelerating
{
	...
	_startTime = CACurrentMediaTime();
	_scrollDuration = fabsf(distance) / fabsf(0.5f * (float)_startVelocity);
...
- (void)step
{
	...
	else if (_decelerating)
	{
		...
		if (time == (CGFloat)_scrollDuration)
		{
			...
			{
				float difference = self.currentItemIndex - _scrollOffset;
				if (difference > 0.5)
				{
