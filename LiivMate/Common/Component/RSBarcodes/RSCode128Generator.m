...
@property (nonatomic, assign) RSCode128GeneratorCodeTable startCodeTable;
...
- (NSUInteger)__startCodeTableValue:(RSCode128GeneratorCodeTable)startCodeTable
{
    ...
    switch (_autoCodeTable.startCodeTable) {
        case RSCode128GeneratorCodeTableA:
            codeTableValue = _codeTableSize - 4;
            break;
        case RSCode128GeneratorCodeTableB:
            codeTableValue = _codeTableSize - 3;
            break;
        case RSCode128GeneratorCodeTableC:
            codeTableValue = _codeTableSize - 2;
            break;
        default:
            break;
    }
...
- (NSUInteger)__middleCodeTableValue:(RSCode128GeneratorCodeTable)codeTable
{
    ...
    switch (codeTable) {
        case RSCode128GeneratorCodeTableA:
            codeTableValue = _codeTableSize - 6;
            break;
        case RSCode128GeneratorCodeTableB:
            codeTableValue = _codeTableSize - 7;
            break;
        case RSCode128GeneratorCodeTableC:
            codeTableValue = _codeTableSize - 8;
            break;
        default:
            break;
    }
                          ...
                          continousDigitsRange:(NSRange)range
{
    ...
    if ((range.location == 0 && range.length >= 4)
        ...
        if (range.location == 0) {
            _autoCodeTable.startCodeTable = RSCode128GeneratorCodeTableC;
        } else {
            if (range.length % 2 == 1) {
                ...
                [_autoCodeTable.sequence addObject:[NSNumber numberWithInteger:digitValue]];
                isOrphanDigitUsed = YES;
            }
            [_autoCodeTable.sequence addObject:[NSNumber numberWithInteger:[self __middleCodeTableValue:RSCode128GeneratorCodeTableC]]];
        }
        ...
        for (int i = 0; i < range.length / 2; i++) {
            ...
            [_autoCodeTable.sequence addObject:[NSNumber numberWithInt:digitValue]];
        }
        
        if ((range.length % 2 == 1 && !isOrphanDigitUsed)
            || !isFinished) {
            [_autoCodeTable.sequence addObject:[NSNumber numberWithInteger:[self __middleCodeTableValue:defaultCodeTable]]];
        }
        ...
        if (!isFinished) {
            ...
            [_autoCodeTable.sequence addObject:[NSNumber numberWithInteger:characterValue]];
        }
    } else {
...
- (void)__calculateAutoCodeTableWithContents:(NSString *)contents
{
    if (self.codeTable == RSCode128GeneratorCodeTableAuto) {
        ...
        _autoCodeTable = [[RSAutoCodeTable alloc] init];
        ...
        if (_autoCodeTable.startCodeTable == RSCode128GeneratorCodeTableAuto) {
            _autoCodeTable.startCodeTable = defaultCodeTable;
        }
    }
}
...
- (id)initWithContents:(NSString *)contents
{
    ...
    if (self) {
        ...
        _codeTableSize = (NSUInteger)(sizeof(CODE128_CHARACTER_ENCODINGS) / sizeof(NSString *));
    }
    return self;
}
...
- (NSString *)initiator
{
    ...
    switch (self.codeTable) {
        case RSCode128GeneratorCodeTableAuto:
            initiator = CODE128_CHARACTER_ENCODINGS[[self __startCodeTableValue:_autoCodeTable.startCodeTable]];
            break;
        case RSCode128GeneratorCodeTableA:
        case RSCode128GeneratorCodeTableB:
        case RSCode128GeneratorCodeTableC:
...
- (NSString *)terminator
{
    return [NSString stringWithFormat:@"%@%@", CODE128_CHARACTER_ENCODINGS[_codeTableSize - 1], @"11"];
}

- (NSString *)barcode:(NSString *)contents
{
    NSMutableString *barcode = [NSMutableString stringWithString:@""];
    
    switch (self.codeTable) {
        case RSCode128GeneratorCodeTableAuto:
            for (int i = 0; i < _autoCodeTable.sequence.count; i++) {
                [barcode appendString:CODE128_CHARACTER_ENCODINGS[[_autoCodeTable.sequence[i] intValue]]];
            }
            break;
        case RSCode128GeneratorCodeTableA:
        case RSCode128GeneratorCodeTableB:
...
- (NSString *)checkDigit:(NSString *)contents
{
    ...
    switch (self.codeTable) {
        case RSCode128GeneratorCodeTableAuto:
            sum += [self __startCodeTableValue:_autoCodeTable.startCodeTable];
            for (int i = 0; i < _autoCodeTable.sequence.count; i++) {
                sum += [_autoCodeTable.sequence[i] intValue] * (i + 1);
            }
            break;
        case RSCode128GeneratorCodeTableA:
            sum = -1; // START A = _codeTableSize - 4 = START B - 1
        case RSCode128GeneratorCodeTableB:
            sum += _codeTableSize - 3; // START B
            for (int i = 0; i < contents.length; i++) {
