...
NSString * const __CODE39_ALPHABET_STRING = @"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-. $/+%*";
...
{
    return CODE39_CHARACTER_ENCODINGS[[__CODE39_ALPHABET_STRING rangeOfString:character].location];
}
...
- (BOOL)isContentsValid:(NSString *)contents
{
    if (contents.length > 0 && [contents isEqualToString:[contents uppercaseString]]) {
        for (int i = 0; i < contents.length; i++) {
            if ([__CODE39_ALPHABET_STRING rangeOfString:[contents substringWithRange:NSMakeRange(i, 1)]].location == NSNotFound) {
                return NO;
            }
        }
        return YES;
