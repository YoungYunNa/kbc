...
- (NSString *)checkDigit:(NSString *)contents
{
    ...
    for (int i = 0; i < contents.length; i++) {
        ...
        sum += [__CODE39_ALPHABET_STRING rangeOfString:character].location;
    }
    // 43 = __CODE39_ALPHABET_STRING's length - 1 -- excludes asterisk
    return [__CODE39_ALPHABET_STRING substringWithRange:NSMakeRange(sum % (__CODE39_ALPHABET_STRING.length - 1), 1)];
}
